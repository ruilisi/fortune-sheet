...
function mouseRender(
  ...
  if (ctx.luckysheet_select_status) {
    ...
    // 解决蓝框跟随问题，获取到当前的sheet是哪个，然后把当前鼠标的选框赋值给sheet
    const currentId: string = ctx.currentSheetId;
    for (let i = 0; i < ctx.luckysheetfile.length; i += 1) {
      if (currentId === ctx.luckysheetfile[i].id) {
        ctx.luckysheetfile[i].luckysheet_select_save =
          ctx.luckysheet_select_save;
        break;
      }
    }
